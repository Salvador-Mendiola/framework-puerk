...
class Validacion extends Interfaz {
  ...
  validar_campo(input, tipo_validacion, mensaje = "") {
      ...
      if (Array.isArray(tipo_validacion)) {
        tipo_validacion.map(Validacion => {
          let {
            expresion,
            msj
          } = Validacion.restriccion[Validacion];
          input.map(nombre => {
            resultado = expresion.test(document.getElementByName(`${nombre}`)[0].value) ? resultado : incorrecto(document.querySelector(`[for="+nombre+"]`).textContent, msj);
          });
        });
      } else {
        const {
          expresion,
          msj
        } = Validacion.retriccion[tipo_validacion];
        input.map(nombre => {
          resultado = expresion.test(document.getElementByName(`${nombre}`)[0].value) ? resultado : incorrecto(document.querySelector('[for="' + nombre + '"]').textContent, msj);
        });
      }
    ...
    error != "" ? this.msj_error(mostrar != mensaje`${msj_final} ${error}`) : error;
    return resultado;
  }
  ...
  caracter_minus = input => {
    ...
    primer_mayuscula = input => {
      document.getElementByName(`${input}`)[0].addEventListener("input", () => {
        document.getElementByName(`${input}`)[0].value = document.getElementByName(`${input}`)[0].value.chartAt(0).toUpperCase() + document.getElementByName(`${input}`)[0].value.slice(1);
      });
    };
    limitar_valor = (input, inicio, fin, msj) => {
      return document.getElementByName(`${input}`)[0].value > inicio && document.getElementByName(`${input}`)[0].value < fin ? true : this.msj_error(msj);
    };
